...
extern kern_return_t start(kmod_info_t *ki, void *data);
extern kern_return_t stop(kmod_info_t *ki, void *data);

// These global symbols will be defined by CreateInfo script's info.c file.
KMOD_EXPLICIT_DECL(AppleHPET, 1, start, stop)

extern kern_return_t _realmain(kmod_info_t *ki, void *data);
extern kern_return_t _antimain(kmod_info_t *ki, void *data);

/* Apple compiler version */
__private_extern__ int _kext_apple_cc = 6000; /* __APPLE_CC__ */

/*********************************************************************
 Initial load routine
 *********************************************************************/
__private_extern__ kern_return_t start(kmod_info_t *ki, void *data)
{
    if (&_realmain)
...
/*********************************************************************
 Initial unload routine
 *********************************************************************/
__private_extern__ kern_return_t stop(kmod_info_t *ki, void *data)
{
    int result = 0;
...
{
    return (OSKextLoadTag)kmod_info.id;
}
